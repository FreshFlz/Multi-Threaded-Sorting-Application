...
public abstract class SortPanel extends JPanel implements Runnable {
	...
	protected void paintComponent(Graphics g) {
		...
		g.fillRect((getWidth() - nameFontMetrix.stringWidth(name)) / 2,
                        0, nameFontMetrix.stringWidth(name),
                        BORDER_WIDTH + nameFontMetrix.getAscent() / 3);
		g.setColor(Color.WHITE);
		g.setFont(nameFont);
		g.drawString(name, (getWidth() - nameFontMetrix.stringWidth(name)) / 2,
                        BORDER_WIDTH + nameFontMetrix.getAscent() / 3);//draw colored bars
